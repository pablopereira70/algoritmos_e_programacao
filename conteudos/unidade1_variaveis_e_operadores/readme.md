# 📝 Módulo 01: Variáveis e Operadores

Bem-vindo ao primeiro módulo da minha Wikipédia técnica! Aqui estão consolidados os fundamentos mais básicos e essenciais para a construção de qualquer software em Python. 

---

## 🗺️ Índice do Módulo

Navegue pelos tópicos abaixo para explorar os conceitos detalhados:

1. [📂 01. Variáveis e Tipos de Dados](./01_variaveis_e_tipos.md)
   * O que é uma variável? (A analogia da caixa).
   * Regras obrigatórias de nomenclatura e padrão *snake_case*.
   * Os 4 tipos de dados primitivos (`int`, `float`, `str`, `bool`).

2. [📂 02. Operadores Matemáticos e Lógicos](./02_operadores.md)
   * Operadores Aritméticos (com foco em Divisão Inteira `//` e Módulo `%`).
   * Operadores Relacionais (Comparações).
   * Operadores Lógicos (`and`, `or`, `not`).

3. [📂 03. Entrada, Saída e o Fluxo de um Algoritmo](./03_entrada_saida_fluxo.md)
   * Capturando dados com `input()` e exibindo dados com `print()` (f-strings).
   * O que é um Algoritmo e seus 3 pilares (Entrada, Processamento e Saída).

---

🎉 **Já terminou?** Acesse o próximo módulo: [Módulo 02: Funções e Escopo](../unidade2_funcoes_e_escopo/).