...
def obter_texto_minimo_e_maximo(conteudo,min, max):
    while True:
        try:
...
def obter_texto_minimo(conteudo,min):
    while True:
        try:
            texto = input(conteudo)
            if len(texto) >= min:
            ...
            print(f"Valor inválido. Isso não é um texto de no mínimo {min} caractere(s)")


def obter_texto_maximo(conteudo, max):
    while True:
        try:
            texto = input(conteudo)
            if len(texto) <= max:
            ...
            print(f"Valor inválido. Isso não é um texto de no máximo {max} caracteres")